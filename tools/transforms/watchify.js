...
//just saved about 2k loc, gg ez
const anymatch = (searchTerm, filepath) => {
  let _path = String(filepath)
  return _path.includes(searchTerm)
}

//uses fs built in watcher.. works great on windows, node docs said something about breaking on some other os
function watchify (b, opts) {
    ...
    function watchFile (file, dep) {
        ...
        var w = fs.watch(path.join(process.cwd(), "build"), {recursive: true});
