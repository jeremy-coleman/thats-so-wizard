  ...
  if (!global._hmr[bundleKey].setStatus) {
    ...
    var moduleHotApply = function(options, cb) {
      ...
      if (localHmr.status !== "ready") {
        cb(
          new Error("module.hot.apply can only be called while status is ready")
        );
        return;
      }
      ...
      //loop -> check 3 conditions -> set new modules, remove modules, or update modules
      for (i = 0, len = acceptedUpdates.length; i < len; i++) {
        an = acceptedUpdates[i];
    ...
    var setupSocket = function() {
      ...
      socket.on("connect", function() {
        ...
        var syncMsg = mapValues(runtimeModuleInfo, (value, name) => {
      ...
      var acceptNewModules = function(msg) {
        ...
        moduleHotApply({ ignoreUnaccepted: ignoreUnaccepted }, (err,updatedNames) => {
          if (err) {
          ...
          if (updatedNames) {
            ...
            if (outdatedModules.length !== updatedNames.length) {
              var notUpdatedNames = filter(outdatedModules, (name) => {
          ...
          accept: function(deps, cb) {
            if (!cb && (!deps || typeof deps === "function")) {
              ...
              for (var i = 0, depsLen = deps.length; i < depsLen; i++) {
                var depIndex =
                  moduleDefs[runtimeModuleInfo[name].index][1][deps[i]];
                if (
                  depIndex === undefined ||
